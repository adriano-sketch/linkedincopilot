...
serve(async (req) => {
  ...
  try {
    ...
    const allowedPassed = remainingCredits > 0 ? allPassed.slice(0, remainingCredits) : [];
    const overflowPassed = remainingCredits > 0 ? allPassed.slice(remainingCredits) : allPassed;

    // Update passed leads (within credit limit)
    if (allowedPassed.length > 0) {
      await supabase
        .from("campaign_leads")
        .update({
          status: "ready",
    ...
    // Leads that passed ICP but exceeded credit limit
    if (overflowPassed.length > 0) {
      await supabase
        .from("campaign_leads")
        .update({
          status: "icp_rejected",
        ...
        .in("id", overflowPassed);
    }

    if (allowedPassed.length > 0) {
      await supabase
        .from("user_settings")
        .update({ leads_used_this_cycle: currentUsed + allowedPassed.length })
        .eq("user_id", userId);
      remainingCredits = Math.max(0, remainingCredits - allowedPassed.length);
    }
    ...
    console.log(`${allowedPassed.length} leads marked as ICP-matched, messages will be generated by cron`);

    console.log(`ICP check done: ${allowedPassed.length} passed, ${allRejected.length + overflowPassed.length} rejected out of ${leads.length}`);

    return new Response(JSON.stringify({
      success: true,
      total_checked: leads.length,
      passed: allowedPassed.length,
      rejected: allRejected.length + overflowPassed.length,
